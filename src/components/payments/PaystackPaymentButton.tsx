...
import { paymentService, PaymentService, PaymentData, PaymentResponse, CreditPackage } from '@/services/paymentService'
...
export const PaystackPaymentButton = ({
  ...
  const [paymentConfig, setPaymentConfig] = useState<PaymentData | null>(null)

  // Prepare payment config at component level
  const preparePaymentConfig = (): PaymentData | null => {
  ...
  // Initialize Paystack payment hook at component level
  const initializePayment = usePaystackPayment({
    reference: paymentConfig?.reference || PaymentService.generateReference(`credits_${creditPackage.id}`),
    email: user?.email || '',
    amount: PaymentService.nairaToKobo(creditPackage.price),
  ...
  const handlePayment = () => {
    ...
    try {
      const config = preparePaymentConfig()
      if (!config) {
        throw new Error('Unable to prepare payment configuration')
      }
